 ...
 *
 *  - "By activity type" and "By GHG Scope" use percentages of the
 *    total emission so the bars share a 0..100 axis.
 *  - "Monthly emissions" uses a relative bar scaled against the
 *    peak month so month-over-month rhythm is readable even when the
 *    absolute totals are small. This is purely a visual choice and
 *    is computed in this component (no domain math).
 *
  ...
  return (
    <Card aria-labelledby={sectionId}>
      ...
      <ul className="mt-4 space-y-4">
        {rows.map((row) => (
          <li key={row.label}>
            <BreakdownRow row={row} isHighlighted={row.label === highlightedLabel} />
          </li>
        ))}
      </ul>
    </Card>
